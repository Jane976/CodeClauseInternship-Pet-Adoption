...
pets.forEach(pets => {
    ...
    img.src  = pets.photo
    img.alt = `A ${pets.species} named ${pets.name}`

    const age = new Date(). getFullYear() - pets.birthYear
    const ageText = decideAgeText(age)
    clone.querySelector(".age"). textContent = ageText

    clone.querySelector(".species").textContent = pets.species
    clone.querySelector(".description"). textContent = pets.description
    clone.querySelector(".name").textContent = pets.name
    clone.querySelector(".primary-btn").href = `https://learnwebcode.github.io/pet-adoption-data/pets/${pets.id}/`
